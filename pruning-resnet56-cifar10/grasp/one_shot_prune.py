...
import models
from pruners.grasp import grasp_mask
from pruners.utils import apply_mask_

def build_loaders(dataset: str, batch_size: int, cuda: bool):
    ...
    return torch.utils.data.DataLoader(train_ds, batch_size=batch_size, shuffle=True, **kwargs)
...
def main():
    parser = argparse.ArgumentParser("GraSP one-shot pruning (unstructured) for CIFAR")
    ...
    parser.add_argument('--save', type=str, default='./logs/grasp_run')
    ...
    train_loader = build_loaders(args.dataset, args.batch_size, cuda)
    ...
    for i, (data, target) in enumerate(train_loader):
        ...
        m = grasp_mask(model, data, target, sparsity=args.sparsity)
